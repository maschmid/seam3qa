...
public class FormValidationFieldProducer {
    ...
    @Produces
    @Dependent
    public <T> InputElement<T> produceInputElement(final InjectionPoint ip) {
        if (isInitialized()) {
            ...
            InputElement<T> inputElementResult = new InputElement<T>(id, component.getClientId(context), component);
